...
public final class Directory implements ZeppelinFile {
    ...
    public Directory initializeDirectory(Path pathToVisit, ConcurrentHashMap<Path, ZeppelinFile> existingFiles)
        ...
        Map<Path, ZeppelinFile> children = new HashMap<>();
        ...
        Files.walkFileTree(pathToVisit, new SimpleFileVisitor<Path>() {
            ...
            public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs) {
                ...
                try {
                    ...
                    if (children.containsKey(dir)) {
                        childrenOfChildDirectory.putAll(children.get(dir).children());
                    }
                    Directory childDirectory = initializeDirectory(dir, childrenOfChildDirectory);
                    children.put(childDirectory.path(), childDirectory);
                }
            ...
            public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) {
                if (!children.containsKey(file)) {
                    children.put(file, new UnloadedNotebook(file));
                }
                return FileVisitResult.CONTINUE;
            }
        ...
        Directory root = new Directory(pathToVisit, children);
        return root;
    }
